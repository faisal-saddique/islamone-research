...
export function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-neutral-800/50 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-neutral-50 rounded-lg p-6 w-full max-w-sm border border-neutral-300" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleEmailSignIn} className="space-y-3 mb-3">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-2 bg-primary hover:bg-primary-light disabled:opacity-50 text-neutral-50 rounded transition-colors"
          >
            {isLoading ? (
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-neutral-50 mx-auto"></div>
            ) : (
              'Sign In'
            )}
          </button>
